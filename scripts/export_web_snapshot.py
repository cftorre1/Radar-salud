...
def _load_exclusions():
    p=Path("config/editorial_exclusions.json")
    if not p.exists():return {}
    try:return json.loads(p.read_text(encoding="utf-8"))
    except Exception:return {}

def _excluded_for_conflict(s):
    cfg=_load_exclusions()
    entities=cfg.get("protected_entities",[])
    adverse=cfg.get("adverse_terms",[])
    allowed=cfg.get("allowed_public_data_terms",[])
    text=" ".join(str(s.get(k,"") or "") for k in ("title","what_happened","why_it_matters","source_name")).lower()
    if not any(e.lower() in text for e in entities):return False
    # Public statistics about the protected entity remain publishable unless the
    # item is clearly adverse/legal/reputational.
    if any(a.lower() in text for a in adverse):return True
    return False
...
def _group(items):
    ...
    base.update({"title":f"Actualización mensual del sistema Isapre — {p}",
    ...
    # Aggregate spreadsheet insights from component sources.
    ins=[]
    for s in best:
        for x in s.get("data_insights",[]) or []:
            if x not in ins:ins.append(x)
    base["data_insights"]=ins[:3]
    return [base]
def _tag_defaults(s):
    r=dict(s)
    ...
    if not r.get("scopes"):
        dom=r.get("system_domain");r["scopes"]=[{"HEALTH_INSURANCE":"Isapres","OCCUPATIONAL_HEALTH":"Salud laboral","PUBLIC_HEALTH":"Salud pública"}.get(dom,"Sistema de salud")]
    return r

def curate(signals,min_local=7,max_local=10):
    clean=[]
    for s in signals:
        if _garbage(s) or s.get("validation_status")=="human_review_required" or _excluded_for_conflict(s):continue
        # A current-news homepage must not display undated items as if they were current.
        if s.get("source_name") in ("Ministerio de Salud","Diario Financiero","SUSESO") and not _d(s.get("event_date")):continue
        clean.append(_tag_defaults(s))
    if not clean:return []
    latest=_latest(clean);current=[s for s in clean if _d(s.get("event_date")) and (latest-_d(s.get("event_date"))).days<=21]
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--input",required=True);ap.add_argument("--output",default="web/data/radar_today.json");ap.add_argument("--date",default=None);args=ap.parse_args()
