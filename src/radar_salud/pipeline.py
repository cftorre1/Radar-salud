...
CATEGORY_KEYWORDS = {
    ...
    "Regulación & Legal": ["circular", "resolución", "corte", "recurso", "demanda", "ley"],
...
def build_signal(raw: RawItem, source_cfg: Dict[str, Any]) -> Signal:
    """
    V0 deliberadamente simple y determinístico.
    En V1, Extractor/Analyst/Validator serán servicios separados y podrán
    usar LLM structured output + verificaciones de fuente.
    """
    ...
    scores = ScoreInputs(
        economic_impact=dimensions.get("economic", 50),
        regulatory_impact=dimensions.get("regulatory", 30),
        scope=dimensions.get("scope", 50),
        novelty=dimensions.get("novelty", 70),
        actionability=dimensions.get("actionability", 50),
    ...
    return Signal(
        ...
        source_url=raw.url,
        category=category,
        subcategory=raw.metadata.get("subcategory", "General"),
        system_domain=source_cfg.get("system_domain", "HEALTH"),
        what_happened=raw.metadata.get("what_happened", raw.raw_text[:350]),
        event_date=raw.event_date,
        topics=raw.metadata.get("topics", []),
        entities=raw.metadata.get("entities", []),
        geography=raw.metadata.get("geography", []),
        key_facts=raw.metadata.get("key_facts", []),
        key_numbers=raw.metadata.get("key_numbers", []),
        why_it_matters=raw.metadata.get("why_it_matters", ""),
        who_cares=raw.metadata.get("who_cares", []),
        possible_implications=raw.metadata.get("possible_implications", []),
        watch_tags=raw.metadata.get("watch_tags", []),
        event_type=raw.metadata.get("event_type", "OTHER"),
        strategic_theme=raw.metadata.get("strategic_theme", []),
        institution_types=raw.metadata.get("institution_types", []),
        country=raw.metadata.get("country", "CL"),
        connection_keys=raw.metadata.get("connection_keys", []),
        ...
        regulatory_impact_score=scores.regulatory_impact,
