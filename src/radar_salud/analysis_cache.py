...
def seed_from_history(root: Path) -> int:
    """Preserve the Terra work already paid for by extracting rich normative analyses from history."""
    ...
    for s in signals:
        ...
        cache[k]={
          ...
          "analysis":{
            ...
            "validity_text":s.get("validity_text"),
            "key_points":s.get("key_points") or [],
            "review_points":s.get("risk_notes") or [],
            "references":s.get("related_reference_ids") or [],
          },
        }
        added+=1
    save_cache(root,cache)
