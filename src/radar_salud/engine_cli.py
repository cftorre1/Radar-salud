...
def _collect(root,name,items,processor,cfg,reset,limit):
    ...
    fresh=_new(items,SeenStore(state))
    rows=[]
    ...
    print(f"{name}: discovered={len(items)} new={len(fresh)} published={len(rows)}")
    return rows
...
def main():
    ...
    for raw in items[:args.limit]:
        try:
            ...
            row["signal_types"]=["Datos"];row["scopes"]=["Isapres"];row["data_insights"]=[];stats.append(row)
        except Exception as e:print("stats:",e)
    all_new+=stats

    # Cache makes rebuild safe for already analyzed SuperSalud PDFs.
    ...
    do=_collect(root,"diario_oficial",DiarioOficialHealthScout().discover(days_back=90 if args.reset_state else 8),process_diario_oficial,cfgs["diario_oficial"],args.reset_state,args.limit*3);all_new+=do

    history=merge_history(history,all_new);save_history(history_path,history)
    print(f"FINAL Stats={len(stats)} SuperNorm={len(reg)} MINSAL={len(mi)} SUSESO={len(su)} DF={len(df)} DO={len(do)} History={len(history)}")
