...
### Fuentes Chile · captura controlada del 24 septiembre

El conector ISP/ANAMED exige fecha válida en una celda de la fila, PDF del dominio oficial rotulado «Publicación ISP» y evaluación editorial antes de publicar. El primer candidato `95c102d` falló la revisión independiente: podía usar una fecha del título o seleccionar un antecedente. Se corrigió en `b3d679e`; el Reviewer detectó además que una tabla ajena con fecha podía parecer ausencia normal de alertas. `059447995561647bcb814e15430ed1ae744ddcc2` falla técnicamente ante estructura no reconocida; [preview #74](https://github.com/cftorre1/Radar-salud/actions/runs/36030110235) pasó tests, QA desktop/mobile, Reviewer y despliegue. Esta es **validación técnica del conector**, no validación de la fuente real.

[Daily Radar pilot #23](https://github.com/cftorre1/Radar-salud/actions/runs/36029771353), limitado a un procesamiento, produjo datos reales en staging `83f55e36`: ISP/ANAMED `URLError` y FONASA `HTTPError`, ambas con 0 descubrimientos/publicaciones y resultado editorial `not_evaluated`. Hubo siete fuentes técnicamente disponibles y dos con error. No se inventó una fila ANAMED ni un evento LIVE. La captura real de estas dos fuentes sigue pendiente; DEIS y newsrooms corporativos continúan en trabajo aprobado.

### Fuentes Chile · piloto #24 y reparación (24 septiembre)

El [piloto #24](https://github.com/cftorre1/Radar-salud/actions/runs/36033189069), limitado a un procesamiento, detectó 15 entradas Bupa: una publicación BACKFILL, dos BACKFILL pendientes y doce descartadas. LIVE permaneció en cero. DEIS no encontró publicación elegible; FONASA devolvió HTTPError, ISP/ANAMED URLError y RedSalud RuntimeError. No se atribuyeron señales a las fuentes con error.

El Reviewer independiente falló el conector DEIS aecc73a por confundir fecha de período y publicación. El guard de de059bb [pasó #81](https://github.com/cftorre1/Radar-salud/actions/runs/36032622335). El [preview #82](https://github.com/cftorre1/Radar-salud/actions/runs/36033669663) detuvo el despliegue por card móvil de 544 px (umbral <420). Se sintetizó la resolución TEA y corrigieron alcance y vigencia del anuncio Bupa. El [preview #85](https://github.com/cftorre1/Radar-salud/actions/runs/36034992656) detectó historial JSON truncado por la publicación API; se restauró el blob íntegro 840a1887 con 680 señales.

El [preview #89](https://github.com/cftorre1/Radar-salud/actions/runs/36035724259), SHA 11f0afe73ef28d2df1df2534513071b1f44e1373, pasó tests, QA desktop 1440×900, mobile 390×844, Reviewer CI y despliegue. Bupa se reproduce desde historia canónica. Fuentes Chile sigue Implementado: FONASA/ISP/RedSalud necesitan captura real y DEIS una publicación elegible. main y producción sin cambios.

### Corte de evidencia Global y Fuentes Chile · 24 septiembre

La muestra Global Lite tiene tres temas verificables (Reuters, Deloitte y McKinsey), separa hipótesis de señales chilenas, declara que la muestra es pública y verifica la igualdad del JSON canónico con el publicado. El [preview #96](https://github.com/cftorre1/Radar-salud/actions/runs/36038753641) pasó tests, browser QA desktop/mobile, Reviewer de CI y despliegue. El Reviewer independiente confirmó el PASS **de la muestra**; dejó el bloque general **Implementado**, pues no existe evidencia suficiente de cobertura/no elegibilidad de otros editores aprobados, monitoreo recurrente ni control de acceso PREMIUM. No se infiere exclusividad a partir del rótulo comercial.

El [piloto #25](https://github.com/cftorre1/Radar-salud/actions/runs/36037182719) detectó nueve publicaciones RedSalud y publicó una señal factual **BACKFILL**. [Preview #93](https://github.com/cftorre1/Radar-salud/actions/runs/36037499154) bloqueó el deploy por altura móvil; [preview #95](https://github.com/cftorre1/Radar-salud/actions/runs/36038145561) pasó tests, QA desktop/mobile, Reviewer y preview tras corregir la card. Bupa y RedSalud aportan una señal BACKFILL cada una; LIVE genuino observado sigue en cero. FONASA reportó HTTPError, ISP/ANAMED URLError y DEIS cero publicaciones elegibles, sin atribuir esos resultados a una selección editorial. Quedan pendientes validación externa de FONASA/ISP y newsrooms farma/healthtech.

### Email técnico y estado humano · 24 septiembre

[Preview #101](https://github.com/cftorre1/Radar-salud/actions/runs/36040290271) falló por incompatibilidad de pruebas al integrar el módulo de correo; el deploy no ocurrió. El [preview #105](https://github.com/cftorre1/Radar-salud/actions/runs/36041103869), SHA `be1e988153db46c7e57bf802c386a75eaa020219`, pasó tests, QA browser desktop/mobile, Reviewer CI y despliegue después de corregir tests, puntajes inválidos y la función que antes completaba LIVE cuando faltaba el modo. El Reviewer independiente revisó las protecciones de privacidad y las transiciones de estados; el bloque Email sigue **Bloqueado** para captura y envío reales. Las banderas `capture_enabled`, `send_enabled` y `privacy_approved` siguen falsas. No se configuraron credenciales, no se capturó PII y no se envió correo. Falta aprobación legal de textos/finalidad/retención, proveedor/cuenta/identidad del remitente y credencial vía GitHub secret con activación explícita.

### Global Lite y Analytics · evidencia del 24 septiembre

[Preview #109](https://github.com/cftorre1/Radar-salud/actions/runs/36042555130) validó la referencia PwC y [preview #111](https://github.com/cftorre1/Radar-salud/actions/runs/36043375959) la referencia WHO/OMS, ambas con QA desktop/mobile, Reviewer y preview. [#110](https://github.com/cftorre1/Radar-salud/actions/runs/36042938827) bloqueó un selector de QA ambiguo antes del deploy; #111 lo corrigió. La muestra pública tiene tres temas y cinco referencias curadas; no equivale a monitoreo recurrente ni acceso exclusivo PREMIUM. CB Insights y OPS/PAHO respondieron 403 durante verificación pública, y una publicación BCG revisada prohíbe uso en bases/productos comerciales; no se integró sin permiso.

El [preview #112](https://github.com/cftorre1/Radar-salud/actions/runs/36043956593) pasó QA desktop/mobile y Reviewer con instrumentación Analytics apagada. El [preview #113](https://github.com/cftorre1/Radar-salud/actions/runs/36044320155) pasó la prueba de payload mínimo usando un proveedor simulado interceptado. `enabled` y `privacy_approved` siguen falsos: cero medición real, cero envío de eventos, cero conversión confirmada. Analytics queda **Bloqueado** para activación hasta decisión humana sobre proveedor y tratamiento, incluidos IP/origen de transporte.

### Presencia y confianza · evidencia del 24 septiembre

El [preview #115](https://github.com/cftorre1/Radar-salud/actions/runs/36045338679) sobre `92effcfeebb4bb312e578475a911ec0d1f5792e0` pasó tests, 16/16 pruebas de navegador desktop 1440×900 y mobile 390×844, Reviewer y publicación staging. Metadatos Open Graph en cuatro vistas y favicon existente quedan validados como subrequisito; la imagen OG respondió HTTP 200. El Reviewer independiente detectó que los borradores legales versionados son visibles en el repositorio público. Se precisó expresamente su visibilidad, ausencia de vigencia y ausencia de páginas legales publicadas en Pages. Faltan aprobación legal, dominio y acceso DNS, SSL propio, correo y contacto institucional; el bloque permanece **Bloqueado**, sin afirmar que el servicio esté disponible ni activar formularios o analytics. Ningún cambio en main o producción.
