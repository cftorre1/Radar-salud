...
fetch('../data/product.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP');return r.json()}).then(d=>{
  ...
  for(const source of Object.values(d.sources)){const row=node('tr','');const checked=source.checked_at?new Date(source.checked_at):null;const last=checked&&!Number.isNaN(checked.getTime())?checked.toLocaleString('es-CL'):'Sin medir';for(const v of [source.name,source.status,last,number(source.live_pending),number(source.backfill_pending),number(source.rejected)])row.append(node('td',v));document.getElementById('sources').append(row)}
  ...
  const familyEvidence=document.getElementById('excelValidation');for(const item of Object.values(d.excel_validation?.families||{})){const lineItem=node('p',`${item.family}: ${item.status} · ${item.series?.length||0} períodos · SHA-256 ${item.sha256||'sin archivo validado'} · `);if(item.source_url)link(lineItem,item.source_url,'Workbook oficial');familyEvidence.append(lineItem)}
