...
fetch('../data/product.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP');return r.json()}).then(d=>{
  ...
  const featureUsage=document.getElementById('featureUsage'),featureSummary=document.getElementById('featureSummary');for(const feature of Object.values(d.usage.features||{})){const total=feature.periods?.accumulated||{};line(featureSummary,`${feature.label}: ${feature.deterministic_outputs||0} output determinístico trazable · modelo ${feature.model_status==='not_applicable_deterministic'?'no aplica':feature.model_used||'No disponible'} · costo/output ${total.cost_per_output??'No disponible'} · costo/pieza publicable ${total.cost_per_publishable??'No disponible'} · costo/punto incremental ${total.cost_per_incremental_quality_point??'No disponible'}.`);for(const period of Object.values(feature.periods||{})){for(const [model,row] of Object.entries(period.models||{})){const tr=node('tr','');for(const value of [feature.label,period.label,model.replace('gpt-5.6-','').replace('gpt-6-',''),row.calls==null?'No disponible':number(row.calls),row.input_tokens==null?'No disponible':number(row.input_tokens),row.output_tokens==null?'No disponible':number(row.output_tokens),row.cost_usd==null?'No disponible':row.cost_usd])tr.append(node('td',value));featureUsage.append(tr)}}}
  ...
  const analytic=d.excel_insights_v1;if(analytic)familyEvidence.append(node('p',analytic.status==='validated'?`Insights V1: ${number(analytic.count)} cálculos determinísticos validados hasta ${analytic.period}; variaciones, acumulados y rachas reconciliados. Anomalías: ${Object.values(analytic.anomaly_checks||{}).flatMap(x=>x.flagged_periods||[]).length} períodos marcados bajo regla explícita; ausencia de marca no demuestra ausencia de anomalías.`:`Insights V1: ${analytic.status}. Sin publicar conclusiones.`));
